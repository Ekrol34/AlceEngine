...
bool CORE::SetWindowIcon(String file)
{
    ...
    {
        Debug.Warning("Alce::SetWindowIcon -> Error loading file \"{}\"", {file});
        return false;
    }
...
bool CORE::AddScene(ScenePtr scene)
{
...
bool CORE::RemoveScene(String name)
{
...
bool CORE::SetCurrentScene(String name)
{
...
bool CORE::SetBrightness(int value)
{
    if(value < 0 || value > 100)
    {
        Debug.Warning("Alce::SetBrightness -> value out of range (0-100).");
        return false;
    }
...
void CORE::DrawBrightnessOverlay()
{
    ...
    brightnessOverlay.setPosition(0.f, 0.f);
    brightnessOverlay.setSize(sf::Vector2f(winSize));
 
    if (brightness < 50)
    {
        float t = (50 - brightness) / 50.0f;
        sf::Uint8 alpha = static_cast<sf::Uint8>(t * 255.f);
 
        brightnessOverlay.setFillColor(sf::Color(0, 0, 0, alpha));
 
        sf::RenderStates states;
        states.blendMode = sf::BlendAlpha;
        window.draw(brightnessOverlay, states);
    }
    else 
    {
        float t = (brightness - 50) / 50.0f;
        sf::Uint8 alpha = static_cast<sf::Uint8>(t * 120.f);
 
        brightnessOverlay.setFillColor(sf::Color(255, 255, 255, alpha));
