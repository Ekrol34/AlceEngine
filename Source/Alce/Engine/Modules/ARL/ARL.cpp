...
void ARL_PROCESSOR::Process(String command)
{
    ...
    if (mainCmd == "help") 
    {
        ...
        else if(subCmd == "bright")
        {
            Debug.ARLMessage(ARLM.helpBright);
        }
        else if(subCmd == "master")
        {
            Debug.ARLMessage(ARLM.helpMaster);
        }
    ...
    else if (mainCmd == "bright")
    {
        if(args.Length() < 2)
        {
            Debug.ARLError("Syntax error, please check out 'help bright' for more info.");
            return;
        }

        int value = args[1].ParseInt();
        
        if(Alce.SetBrightness(value))
        {
            Debug.ARLMessage("Brightness set to {}", {value});
        }
    }
    else if (mainCmd == "master")
    {
        if(args.Length() < 2)
        {
            Debug.ARLError("Syntax error, please check out 'help master' for more info.");
            return;
        }

        int value = args[1].ParseInt();
        
        if(Audio.SetMasterVolume(value))
        {
            Debug.ARLMessage("Master volume set to {}", {value});
        }
    }
    else if (mainCmd == "has") 
    {
